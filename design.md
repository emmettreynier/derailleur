...
- **`bin/install.sh`** — host bootstrap: checks deps (`gh`, `jq`, `claude`, `python3`, `git`) + `gh` auth, marks the in-repo host hooks + launcher scripts executable, creates `logs/`+`state/`, and scaffolds `orchestrator.conf` from the tracked example. It deliberately registers **nothing** in `~/.claude/` — the launchers pass the host hooks by in-repo path (`--settings`/hook flags), so interactive Claude Code sessions elsewhere on the machine stay untouched. The `launchd` plist is installed separately by `bin/schedule.sh install` (needs sudo, so it's opt-in). (The new-machine sequence — clone → `install.sh` → `gh auth login` → fill `orchestrator.conf` — is in the [README](README.md) *Install*.)
