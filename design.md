...
**Compute persistence — the complement to the hard-stop handling above.** `finalize_dispatch` rescues *commit* state when a worker dies, but in-flight *compute* (a multi-hour estimation kicked off inline) dies with the process. `briefs/worker-brief.md` closes that gap with a convention (no new `bin/` helper): a worker runs any long command detached in a tmux session whose name is a pure function of the task — `derail-<repo-slug>-<issue>`, repo+issue only, no timestamps/PIDs — so the run survives worker death and the next worker reattaches instead of restarting from zero. The same canonical name doubles as a cross-worker mutex: `tmux new-session -d -s <name>` succeeds for exactly one worker; a second worker's create fails on the name collision and falls through to reconcile (tail the log, check for outputs, then wait / reuse / kill+relaunch) rather than spawning a colliding duplicate. Consistent with "GitHub is the only source of truth," the runtime session record is a **comment** on the issue/PR (session name, command, durable log path) — never a local state file, since the name is recomputable and `tmux ls` is the runtime truth for liveness. Because worktrees get pruned (`bin/worktree-prune.sh`), long jobs write both their results and their tee'd log to durable paths outside the worktree.
...
- **`bin/smoke-test.sh`** — verifies the bootstrap itself: it exercises the `config-common.sh` identity guard against throwaway confs in all three states (missing → abort; one field blank → abort *naming the field*; all five filled → exit 0 + vars exported) and confirms a filled conf renders `{{OPERATOR_NAME}}` into a brief the way the launchers do, so a regression in the guard or the scaffold step surfaces here rather than on a labmate's first real dispatch (or, worse, as work dispatched under the maintainer's identity). It never mutates a real `orchestrator.conf` (throwaway temp confs; asserts the real one is byte-identical entry→exit) and `SKIP:`s any `gh`/network step so it stays deterministic offline. (README → *Verify / smoke test*.)
- **The `~/.claude/` carve-out — `/orchestrate`.** The one thing derailleur writes under `~/.claude/` is a slash-command file, `~/.claude/commands/orchestrate.md`, rendered by `install.sh` from the tracked template `templates/orchestrate.command.md` with this checkout's absolute path baked into `{{DERAILLEUR_ROOT}}` (idempotent; rewrites only on content change). It is **safe by construction**: a command file is *inert until the operator types `/orchestrate`* — it registers no `SessionStart`/`PreToolUse`/`Stop` hook, so it alters no default session behavior anywhere; its `allowed-tools` is scoped to exactly the board digest, the two launchers, and read-only `gh` (never blanket `Bash`); and it can only *propose* — the human-gated brief requires an explicit confirmation before any dispatch, and nothing here can merge. This is why the carve-out doesn't violate the worker-hook-isolation principle above: worker *safety* hooks still never leak into normal sessions; only an opt-in, read-mostly convenience command does. One portability guard on that render: because `orchestrate.md` bakes in an absolute path it must never be committed, so `install.sh` resolves the render target through symlinks (`pwd -P`) and warns loudly when it lands inside a git work tree — some dotfiles setups symlink `~/.claude/commands` into a versioned repo, and a committed copy would carry one machine's home directory and dead-end `/orchestrate` on every other machine that pulls it. It warns rather than hard-fails (the file renders fine; the danger only materializes on commit). (README → *Install*.)
