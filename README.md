...
`conflicting` rung: an explicit `CONFLICTING` asserts, while `UNKNOWN`-then-`MERGEABLE`,
a stuck `UNKNOWN`, a `gh` failure and unparseable output all stay `done` with a note,
the re-poll stays bounded, and a draft PR reports `draft` without any mergeability call),
and both checker round comments it posts (`**Checker interrupted:` /
`**Checker incomplete:`, and nothing at all on a clean finish); the checker-round cap
...
`WORKER_LIMIT`; a mixed run tallies each class independently, an intervening reply
resets both, and a run that trips one class while the other has a non-zero count gets
...
same three `gh` calls); and **manifest-parser conformance** — the deny-hook
(`host/hooks/raw-data-guard.py`) and the launchers' `yml_list()` are two implementations
of one manifest grammar, and when they disagree the hook silently drops a declared
...
| `WORKER_LIMIT` | `4` | Max consecutive worker attempts with no clean finish before escalating to `needs-input` — counts every `interrupted-*` (cut off) and every `incomplete-*` reason *except* `waiting` (so `uncommitted`, `unpushed`, `nopr`, `draft`, `conflicting`). |
