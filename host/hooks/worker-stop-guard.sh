...
block "You haven't left a trail, so this work would be invisible to the rest of the loop. Before finishing, do ONE of: (a) open a PR for this branch referencing \"Closes #$ISSUE\" with the results-summary section filled, or (b) comment the specific obstacle on #$ISSUE and add the \"needs-input\" label. (b) is a legitimate, SUCCESSFUL finish, not a failure — use it for a research/judgment call, for a retrieval/tool/access failure (the data does not exist, the run will not complete), or for a criterion that is unachievable as written; a PR that honestly reports an unmet criterion is equally fine. NEVER invent a result, a number, or a command output to satisfy this hook: a truthful \"I could not do this\" is the right answer and fabrication is not. Then you may stop."
